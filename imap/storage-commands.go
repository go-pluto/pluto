...
func (storage *Storage) Select(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {

	log.Printf("Serving SELECT '%s'...\n", req.Tag)

	// Lock storage exclusively and unlock whenever
	// this handler exits.
	storage.lock.Lock()
	defer storage.lock.Unlock()

	if (storage.Contexts[clientID].IMAPState != AUTHENTICATED) && (storage.Contexts[clientID].IMAPState != MAILBOX) {
	...
	mailboxPath := storage.Contexts[clientID].UserMaildirPath
	...
	storage.Contexts[clientID].IMAPState = MAILBOX
	storage.Contexts[clientID].SelectedMailbox = mailboxes[0]

	// Store contents structure of selected mailbox for
	// later convenient use.
	selMailboxContents := storage.MailboxContents[storage.Contexts[clientID].UserName][mailboxes[0]]
...
func (storage *Storage) Create(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {
	...
	posMailbox := strings.TrimSuffix(posMailboxes[0], storage.Config.IMAP.HierarchySeparator)
	...
	userMainCRDT := storage.MailboxStructure[storage.Contexts[clientID].UserName]["Structure"]
	...
	posMaildir := maildir.Dir(filepath.Join(storage.Contexts[clientID].UserMaildirPath, posMailbox))
	...
	posMailboxCRDTPath := filepath.Join(storage.Contexts[clientID].UserCRDTPath, fmt.Sprintf("%s.log", posMailbox))
	...
	storage.MailboxStructure[storage.Contexts[clientID].UserName][posMailbox] = posMailboxCRDT

	// Initialize contents slice for new mailbox to track
	// message sequence numbers in it.
	storage.MailboxContents[storage.Contexts[clientID].UserName][posMailbox] = make([]string, 0, 6)
	...
	err = userMainCRDT.Add(posMailbox, func(payload string) {
		workerSyncChan <- fmt.Sprintf("create|%s|%s|%s", storage.Contexts[clientID].UserName, base64.StdEncoding.EncodeToString([]byte(posMailbox)), payload)
	})
	if err != nil {
		...
		delete(storage.MailboxStructure[storage.Contexts[clientID].UserName], posMailbox)
		delete(storage.MailboxContents[storage.Contexts[clientID].UserName], posMailbox)
...
func (storage *Storage) Delete(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {
	...
	// Trim supplied mailbox name of hierarchy separator if
	// it was sent with a trailing one.
	delMailbox := strings.TrimSuffix(delMailboxes[0], storage.Config.IMAP.HierarchySeparator)
	...
	err := userMainCRDT.Remove(delMailbox, func(payload string) {
		workerSyncChan <- fmt.Sprintf("delete|%s|%s|%s", storage.Contexts[clientID].UserName, base64.StdEncoding.EncodeToString([]byte(delMailbox)), payload)
	})
	if err != nil {
	...
	// mail contents slice.
	delete(storage.MailboxStructure[storage.Contexts[clientID].UserName], delMailbox)
	delete(storage.MailboxContents[storage.Contexts[clientID].UserName], delMailbox)

	// Construct path to CRDT file to delete.
	delMailboxCRDTPath := filepath.Join(storage.Contexts[clientID].UserCRDTPath, fmt.Sprintf("%s.log", delMailbox))
	...
	// from stable storage.
	delMaildir := maildir.Dir(filepath.Join(storage.Contexts[clientID].UserMaildirPath, delMailbox))
...
func (storage *Storage) List(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {
	...
	// Save user's mailbox structure CRDT to more
	// conveniently use it hereafter.
	userMainCRDT := storage.MailboxStructure[storage.Contexts[clientID].UserName]
	...
	for mailbox, _ := range userMainCRDT {
		...
		if mailbox != "Structure" {
			...
			mailboxParts := strings.Split(mailbox, storage.Config.IMAP.HierarchySeparator)

			if (listArgs[1] == "*") || (len(mailboxParts) == 1) {

				// Either always include a mailbox in the response
				// or only when it is a top level mailbox.
				listAnswerLines = append(listAnswerLines, fmt.Sprintf("* LIST () \"%s\" %s", storage.Config.IMAP.HierarchySeparator, mailbox))
			}
		}
	}
...
func (storage *Storage) Append(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {
	...
	if mailbox == "INBOX" {
		appMaildir = maildir.Dir(storage.Contexts[clientID].UserMaildirPath)
	} else {
		appMaildir = maildir.Dir(filepath.Join(storage.Contexts[clientID].UserMaildirPath, mailbox))
	}
	...
	appMailboxCRDT := storage.MailboxStructure[storage.Contexts[clientID].UserName][mailbox]

	// Append new mail to mailbox' contents CRDT.
	storage.MailboxContents[storage.Contexts[clientID].UserName][mailbox] = append(storage.MailboxContents[storage.Contexts[clientID].UserName][mailbox], mailFileName)
	...
	err = appMailboxCRDT.Add(mailFileName, func(payload string) {
		workerSyncChan <- fmt.Sprintf("append|%s|%s|%s;%s", storage.Contexts[clientID].UserName, base64.StdEncoding.EncodeToString([]byte(mailbox)), payload, base64.StdEncoding.EncodeToString(msgBuffer))
	})
	if err != nil {
...
func (storage *Storage) Expunge(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {
	...
	if storage.Contexts[clientID].IMAPState != MAILBOX {
	...
	expMailboxCRDT := storage.MailboxStructure[storage.Contexts[clientID].UserName][storage.Contexts[clientID].SelectedMailbox]

	// Construct path to Maildir to expunge.
	var expMaildir maildir.Dir
	if storage.Contexts[clientID].SelectedMailbox == "INBOX" {
		expMaildir = maildir.Dir(filepath.Join(storage.Contexts[clientID].UserMaildirPath, "cur"))
	} else {
		expMaildir = maildir.Dir(filepath.Join(storage.Contexts[clientID].UserMaildirPath, storage.Contexts[clientID].SelectedMailbox, "cur"))
	}

	// Save all mails possibly to delete and
	// amount of these files.
	expMails := storage.MailboxContents[storage.Contexts[clientID].UserName][storage.Contexts[clientID].SelectedMailbox]
	...
	if numExpMails > 0 {
		...
		for _, msgNum := range expMailNums {
			...
			err := expMailboxCRDT.Remove(expMails[msgNum], func(payload string) {
				workerSyncChan <- fmt.Sprintf("expunge|%s|%s|%s", storage.Contexts[clientID].UserName, base64.StdEncoding.EncodeToString([]byte(storage.Contexts[clientID].SelectedMailbox)), payload)
			})
			if err != nil {
...
func (storage *Storage) Store(c *Connection, req *Request, clientID string, workerSyncChan chan string) bool {
	...
	msgNums, err := ParseSeqNumbers(storeArgs[0], storage.MailboxContents[storage.Contexts[clientID].UserName][storage.Contexts[clientID].SelectedMailbox])
	if err != nil {
	...
	mailMaildir := maildir.Dir(filepath.Join(storage.Contexts[clientID].UserMaildirPath, storage.Contexts[clientID].SelectedMailbox))
	...
	for _, msgNum := range msgNums {
		...
		mailFileName := storage.MailboxContents[storage.Contexts[clientID].UserName][storage.Contexts[clientID].SelectedMailbox][msgNum]

		// Read message contents from file.
		mailFileContents, err := ioutil.ReadFile(filepath.Join(storage.Contexts[clientID].UserMaildirPath, storage.Contexts[clientID].SelectedMailbox, "cur", mailFileName))
		if err != nil {
		...
		if mailFlags != string(newMailFlags) {
			...
			storeMailboxCRDT := storage.MailboxStructure[storage.Contexts[clientID].UserName][storage.Contexts[clientID].SelectedMailbox]
			...
			err = storeMailboxCRDT.Add(newMailFileName, func(payload string) {
				workerSyncChan <- fmt.Sprintf("store|%s|%s|%s|%s;%s", storage.Contexts[clientID].UserName, base64.StdEncoding.EncodeToString([]byte(storage.Contexts[clientID].SelectedMailbox)), rmvElements, payload, base64.StdEncoding.EncodeToString(mailFileContents))
			})
			if err != nil {
			...
			storage.MailboxContents[storage.Contexts[clientID].UserName][storage.Contexts[clientID].SelectedMailbox][msgNum] = newMailFileName
		}
